            ...
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                ...
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{ route('admin.child-category.index') }}">Child-Category</a>
                        </li>
    ...
    <div class="row">
        <form class="row g-3" action="{{ route('admin.child-category.update', $childCategory->id) }}" method="POST">
            ...
            <div class="col-md-6 has-validation">
                ...
                <input type="text" name="name" class="form-control" id="child_category_name"
                    value="{{ $childCategory->name }}" placeholder="Child-Category Name">
                @error('name')
                    {{ $message }}
                @enderror
            </div>
            <div class="col-md-6 has-validation">
                ...
                <select class="form-select" name="category_id" id="validationCustom04">
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->id }}" {{ $category->id == $childCategory->id ? 'SELECTED' : '' }}>
                ...
                <select class="form-select" name="sub_category_id" id="validationCustom04">
                    ...
                    @foreach ($subCategories as $subCategory)
                        <option value="{{ $subCategory->id }}"
                            {{ $subCategory->id == $childCategory->id ? 'SELECTED' : '' }}>{{ $subCategory->name }}
                        </option>
                    @endforeach
                </select>
