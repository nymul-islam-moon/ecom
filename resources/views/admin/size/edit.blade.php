...
@section('admin_content')
    ...
    @endsession
    <div class="row">
        <form class="row g-3 was-validated" action="{{ route('admin.size.update', $size->id) }}" method="POST" novalidate>
            @csrf
            @method('PUT')
            <div class="col-md-6 has-validation">
                <label for="size_name" class="form-label">Name</label>
                <input type="text" name="name" class="form-control" id="size_name" value="{{ $size->name }}" required placeholder="Size Name">
                @error('name')
